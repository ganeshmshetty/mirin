...
"#;
    ...
    assert!(
        result.is_ok(),
        "Expected Ok(_) because exit code is 0, got: {:?}",
        result
    );
    let msg = result.unwrap();
    assert!(
        msg.contains("Connection refused"),
        "Expected 'Connection refused' in stdout, got: {:?}",
        msg
    );
}
...
"#;
    ...
    assert!(
        result.is_err(),
        "Expected Err for wrong pairing code, got: {:?}",
        result
    );
    let err = result.unwrap_err();
    assert!(
        err.contains("pairing code")
            || err.contains("Failed to pair")
            || err.contains("ADB command failed"),
        "Expected pairing-code error, got: {:?}",
        err
    );
}
...
"#;
    ...
    assert!(
        result.is_ok(),
        "Expected Ok for successful pair, got: {:?}",
        result
    );
...
"#;
    ...
    assert!(
        devices.is_empty(),
        "Expected empty devices list, got: {:?}",
        devices
    );
}
...
"#;
    ...
    assert_eq!(
        services.len(),
        2,
        "Expected 2 mDNS services, got: {:?}",
        services
    );

    let connect = services
        .iter()
        .find(|s| s.service_type.contains("tls-connect"));
    ...
    let pairing = services
        .iter()
        .find(|s| s.service_type.contains("tls-pairing"));
...
"#;
    ...
    assert!(
        result.is_err(),
        "Expected Err when only localhost IP present, got: {:?}",
        result
    );
    let err = result.unwrap_err();
    assert!(
        err.contains("Could not determine device IP") || err.contains("WiFi"),
        "Expected informative error, got: {:?}",
        err
    );
}
...
"#;
    ...
    assert!(
        result.is_ok(),
        "Expected Ok for disconnect, got: {:?}",
        result
    );
...
"#;
    ...
    assert!(
        result.is_ok(),
        "Expected Ok(()) for start-server, got: {:?}",
        result
    );
}
...
"#;
    ...
    assert_eq!(
        services.len(),
        1,
        "Expected 1 service (malformed address filtered)"
    );
...
"#;
    ...
    assert!(
        ...
        "Expected ADB command failed message, got: {:?}",
        err
    );
}
...
"#;
    ...
    let usb = devices
        .iter()
        .find(|d| d.serial == "USB123")
        .expect("USB123 not found");
    assert_eq!(usb.state, "device");

    let wireless = devices
        .iter()
        .find(|d| d.serial == "192.168.5.5:5555")
        .expect("wireless not found");
    assert_eq!(wireless.state, "offline");

    let unauth = devices
        .iter()
        .find(|d| d.serial == "USB_UNAUTH")
        .expect("unauthorized not found");
...
"#;
    ...
    assert!(
        result.is_err(),
        "Expected Err when only link-local IP exists, got: {:?}",
        result
    );
}
