...
use mirin_core::screenshot::ScreenshotResult;
...
pub trait RuntimeHost: Send + Sync {
    ...
    fn capture_screenshot<'a>(
        ...
        serial: &'a str,
        annotate: bool,
    ) -> Pin<Box<dyn Future<Output = Result<ScreenshotResult, String>> + Send + 'a>>;
...
#[derive(Clone)]
pub struct ToolDispatcher<H: RuntimeHost> {
    host: Arc<H>,
...
impl<H: RuntimeHost> ToolDispatcher<H> {
    pub fn new(
        host: Arc<H>,
        state: EmbeddedScrcpyState,
    ...
    {
        Box::pin(async move {
            let adb = Adb::new(self.host.adb_path()?);

            match name {
                "list_devices" => {
                    let adb_path = self.host.adb_path()?;
                    let devices: Vec<mirin_core::device_registry::Device> =
                        self.device_registry.get_resolved_devices(adb_path).await?;
                ...
                "connect_device" => {
                    ...
                    let scrcpy_dir = self.host.scrcpy_dir()?;
                    let scrcpy_version =
                        mirin_core::scrcpy::get_version_number(&scrcpy_path, &scrcpy_dir);
                    let opts = mirin_core::scrcpy::stream::EmbeddedStreamSettings::default();

                    let streams = mirin_core::scrcpy::stream::start_server(
                        &adb,
                        &serial,
                        &scrcpy_server_path,
                        &scrcpy_version,
                    ...
                    if args.get("popup").and_then(|v| v.as_bool()).unwrap_or(false) {
                        ...
                        let _ = self.host.open_mirror_window(serial, model).await;
                    }
                ...
                "get_screenshot" => {
                    ...
                    let res = self
                        .host
                        .capture_screenshot(&self.ui_extractor, &serial, annotate)
                        .await?;
                ...
                "swipe" | "drag" => {
                    ...
                    let duration_ms = args["duration_ms"]
                        .as_u64()
                        .unwrap_or(if is_drag { 800 } else { 300 })
                        .clamp(50, 10000);
                    let (socket, sess_w, sess_h) =
                        self.state.get_session_info(&serial).map_err(|_| {
                            "No mirror session for this device. Call connect_device first."
                                .to_string()
                        })?;
                    ...
                    let (sx, sy, dsp_w, dsp_h) = if args
                        .get("selector")
                        .and_then(|v| v.as_str())
                        .is_some()
                        || args.get("start_x").and_then(|v| v.as_f64()).is_some()
                    {
                        self.resolve_abs_coords(&adb, &serial, &args, sess_w, sess_h)
                            .await?
                    } else {
                    ...
                    let (ex, ey) = {
                        let exv = args
                            .get("end_x")
                            .and_then(|v| v.as_f64())
                            .ok_or("Missing 'end_x' (required for swipe/drag)")?
                            as f32;
                        let eyv = args
                            .get("end_y")
                            .and_then(|v| v.as_f64())
                            .ok_or("Missing 'end_y' (required for swipe/drag)")?
                            as f32;
                        let mode = args
                            .get("coordinate_mode")
                            .and_then(|v| v.as_str())
                    ...
                    for i in 1..=steps {
                        ...
                        control::inject_touch(&socket, "move", cx, cy, dsp_w as u16, dsp_h as u16)
                            .await
                ...
                "scroll_to" => {
                    ...
                    let (socket, w, h) = self.state.get_session_info(&serial).map_err(|_| {
                        "No mirror session for this device. Call connect_device first.".to_string()
                    })?;
                    let w = w.max(1);
                    let h = h.max(1);
                    ...
                    for attempt in 0..=max_swipes {
                        ...
                        for i in 1..=steps {
                            ...
                            let cy = (start_y as f32 + (end_y as f32 - start_y as f32) * t).round()
                                as u32;
                            control::inject_touch(
                                &socket, "move", center_x, cy, w as u16, h as u16,
                            )
                            .await
                            .map_err(|e| format!("Touch move failed: {}", e))?;
                ...
                "press_key" => {
                    ...
                    control::inject_keycode(&socket, "down", keycode, 0, 0)
                        .await
                ...
                "clipboard" => {
                    ...
                    if action == "set" {
                        let text = args["text"]
                            .as_str()
                            .ok_or("Missing 'text' for clipboard set")?;
                        if text.len() > 100000 {
                            return Err("Clipboard text too long (max 100000 bytes)".to_string());
                        }
                        if let Ok(socket) = self.state.get_control_socket(&serial) {
                            control::set_clipboard(&socket, text)
                                .await
                                .map_err(|e| format!("Scrcpy clipboard set failed: {}", e))?;
                        ...
                        let _ = adb
                            .execute(&["shell", "cmd", "clipboard", "set-text", &escaped])
                            .await;
                        ...
                        let verify = adb
                            ...
                            .await;
                        let verify_text = verify
                            .ok()
                            .and_then(|out| parse_clipboard_parcel(&out))
                            .unwrap_or_default();
                        if verify_text == text
                            || (!verify_text.is_empty() && text.contains(&verify_text))
                        {
                        ...
                        if let Ok(socket) = self.state.get_control_socket(&serial) {
                            if let Ok(text) = control::get_clipboard(&socket).await {
                                return Ok(
                                    json!({ "clipboard": text, "method": "scrcpy_control" }),
                                );
                            }
                        }
                ...
                "list_apps" => {
                    ...
                    if packages.is_empty() {
                        return Ok(
                            json!({ "packages": [], "note": "No packages found. Is the device authorized?" }),
                        );
                    }
                ...
                "launch_app" => {
                    ...
                    if let Some(act) = args["activity"].as_str() {
                        ...
                        adb.execute(&["shell", "am", "start", "-n", &comp])
                            .await
                        ...
                        let out = adb
                            ...
                            .await
                            .map_err(|e| {
                                format!("Failed to resolve activity for '{}': {}", pkg, e)
                            })?;
                        ...
                        if resolved.contains('/') && !resolved.contains("No activity found") {
                            adb.execute(&["shell", "am", "start", "-n", resolved])
                                ...
                                .map_err(|e| {
                                    format!(
                                        "Failed to start resolved activity '{}': {}",
                                        resolved, e
                                    )
                                })?;
                        } else {
                ...
                "stop_app" => {
                    ...
                    adb.with_device(&serial)
                        .execute(&["shell", "am", "force-stop", pkg])
                        .await
                ...
                "grant_permission" => {
                    ...
                    if perm.is_empty() {
                        return Err("Permission name must not be empty".to_string());
                    }
                    adb.with_device(&serial)
                        .execute(&["shell", "pm", "grant", pkg, perm])
                        .await
                ...
                "revoke_permission" => {
                    ...
                    adb.with_device(&serial)
                        .execute(&["shell", "pm", "revoke", pkg, perm])
                        .await
                ...
                "run_script" => {
                    ...
                    let steps: Vec<ScriptStep> = serde_json::from_value(steps_val)
                        .map_err(|e| format!("Invalid script steps: {}", e))?;
                            ...
                            Value::Object(mut m) => {
                                ...
                                if step.action == "swipe" || step.action == "drag" {
                                    if let Some(v) = m.remove("x") {
                                        m.insert("start_x".to_string(), v);
                                    }
                                    if let Some(v) = m.remove("y") {
                                        m.insert("start_y".to_string(), v);
                                    }
                                }
                                Value::Object(m)
                            }
    ...
    async fn resolve_abs_coords(
        ...
        let (x, y, source_w, source_h) = if let Some(sel) = args["selector"].as_str() {
            ...
            return Err(
                "Must provide either 'selector' or both 'x' and 'y' coordinates".to_string(),
            );
        };
...
impl<H: RuntimeHost> crate::server::ToolExecutor for ToolDispatcher<H> {
    fn call_tool<'a>(
        &'a self,
        name: &'a str,
        args: Value,
